...
4. If any SQS queues conflict (already exist outside CloudFormation), delete
  them
   with `aws sqs delete-queue`
5. Handle stack rollback states:
   - If a stack is in `ROLLBACK_COMPLETE` state (failed initial creation),
     delete it
     with
...
6. Wait for all operations to complete and verify all stacks are
  CREATE_COMPLETE or
