...
5. **ALWAYS include a status section** at the top with current phase, next steps,
...
6. **ALWAYS include explicit quality requirements** stating: "After completing
   ...
   All code must be well-documented with clear comments explaining business logic and
...
7. **Structure phases based on requirements**:
   ...
   - Phase 4+ (Features/Enhancements) - Only if additional features beyond core requirement
...
13. **End each task description with**: "Run `swift test` and verify quality compliance before marking
...
17. Include commit SHA tracking in the issue template for implementation progress
18. Run the `/format-markdown` Claude command to ensure the markdown is formatted correctly.
